...
public class TechnicianScheduleCommandServiceImpl implements TechnicianScheduleCommandService {
    ...
    public String handle(CreateTechnicianScheduleCommand command) {
        try {
            ...
            Optional<Technician> technicianOpt = technicianRepository.findById(command.technician().getIdTechnician());
            if (technicianOpt.isEmpty()) {
                throw new NotFoundIdException(Technician.class, command.technician().getIdTechnician());
            }
            ...
            return technicianSchedule.getIdTechnicianSchedule();
